#!/usr/bin/env python3
"""S24AA import — attach the 6 Summer LoRA v4 validation images to Summer's image
library as normal character images (kind=SCENE_ONLY, PRIVATE), via the EXISTING
ingestion path (mirrors import_s24t_validation_to_library.py).
...
metadata.source = "summer_lora_v4_validation"; the original filename is preserved.
...
Run from backend/ (so the app env + DATABASE_URL are loaded):
    cd backend && python3 ../scripts/import_s24aa_validation_to_library.py [--commit]
...
SOURCE = "summer_lora_v4_validation"
VAL_DIR = Path(__file__).resolve().parent / "s24aa_validation"
MANIFEST = VAL_DIR / "manifest_status.json"
PROVIDER = "runpod_realvisxl_summer_lora_v4"
EXPECTED = 6
