#!/usr/bin/env python3
"""Sprint 22 — CANON-CARD GROUNDING proof-of-concept driver (one-shot, RunPod).

Copy of the proven founder_async_runpod_driver.py (Sprint 13) that ships the Sprint 22
pod script (comfyui_canon_ground_pod_s22.py): RealVisXL base + the BASE pass visually
grounded on Summer's locked canon cards (face_front + body_front) via the already-loaded
IP-Adapter. Tattoo passes unchanged. ONE validation image; manual founder review.

Deltas vs the Sprint 13 driver:
  - Reads Summer's canon face/body card URLs from character_identity_canon (read-only)
    and forwards them to the pod via FACE_REF_URL / BODY_REF_URL (+ CANON_IP_SCALE,
    BASE_MODEL_ID passthroughs).
  - Safety profile widened for the RealVisXL download: SPEND_CAP=0.07, 900s in-pod
    watchdog, 900s driver wallclock, launch-time RATE GUARD (max safe ~$0.266/hr),
    finally-terminate, orphan verify. Worst-case spend stays < $0.07.
...
SPEND_CAP = 0.07            # S22 one-shot hard ceiling (RealVisXL download headroom)
TERMINATE_AT = 0.065       # external kill threshold (margin below the cap)
WATCHDOG_S = 900           # in-pod self-kill backstop; 900s @ $0.22/hr = $0.055 < cap
DRIVER_WALLCLOCK_S = 900   # driver-side max observed uptime before force-kill
POLL_S = 20
...
def build_report(plan, by_route, pod_id, gpu, st, spend, runtime_s, no_orphans,
    ...
    report = {
        "executor": "runpod_canon_ground_s22",
        "version": "v1",
        "experiment": "canon_card_grounding_poc",
...
def main():
    ...
    run_id = os.environ.get("FOUNDER_RUN_ID") or ("s22_canonground_" + time.strftime(
        "%Y%m%d_%H%M%S", time.gmtime()))
    base_prompt = os.environ.get("BASE_PROMPT") or ""
    canon_ip_scale = os.environ.get("CANON_IP_SCALE", "0.5")
    ...
    CANON_GROUNDING.update({"base_model_id": base_model_id,
                            "canon_ip_scale": float(canon_ip_scale),
    ...
    log(f"canon cards: face={face_ref[-24:]} body={body_ref[-24:]} "
        f"scale={canon_ip_scale} base_model={base_model_id}")
    log(f"max_safe_rate=${MAX_SAFE_RATE}/hr cap=${SPEND_CAP}")

    with open(os.path.join(HERE, "comfyui_canon_ground_pod_s22.py"), "rb") as f:
    ...
    env = {
        ...
        "FACE_REF_URL": face_ref,    # Sprint 22: canon face card → base-pass grounding
        "BODY_REF_URL": body_ref,    # Sprint 22: canon body card → base-pass grounding
        "CANON_IP_SCALE": canon_ip_scale,
