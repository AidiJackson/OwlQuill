#!/usr/bin/env python3
"""Sprint 23B — IDENTITY & QUALITY STACK driver (throwaway, founder-only).

Ships s23b_quality_stack_pod.py: ONE pod, best-of-4 seeds through the full stack —
...
def build_report(plan, by_route, pod_id, gpu, st, spend, runtime_s, no_orphans,
    ...
    report = {
        "executor": "s23b_quality_stack_driver",
        "version": "v1",
        "experiment": "s23b_identity_quality_stack",
...
def main():
    ...
    run_id = os.environ.get("FOUNDER_RUN_ID") or ("s23b_quality_" + time.strftime(
    ...
    CANON_GROUNDING.update({"base_model_id": base_model_id,
                            "stack": "base[face+body@0.5] -> tattoos -> hires1.5x@0.30 "
                                     "-> faceid_refine@0.8/str0.45, best-of-4 seeds",
    ...
    with open(os.path.join(HERE, "s23b_quality_stack_pod.py"), "rb") as f:
