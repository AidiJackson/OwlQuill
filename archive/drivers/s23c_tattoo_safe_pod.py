"""Pod-side Sprint 23C — TATTOO-SAFE HIRES (throwaway, founder-only).

Identical to the Sprint 23B stack with ONE fix: the hires img2img pass re-rendered
tattoo zones (full-white mask, strength 0.30) and destroyed fine linework. Now the
per-candidate feathered arm masks are RETAINED and, after the hires render, the
pre-hires (post-enforcement) pixels are composited back through the upscaled mask
union — the Sprint 14 paste-back guarantee extended through the quality stack.
Tattoo pixels stay canonical; face/body re-detailing keeps its gains.
...
def main():
    ...
    tattoo_masks = {}  # cand -> feathered arm masks (REUSED for tattoo-safe hires)
    for seed in SEEDS:
        cand = f"seed{seed}"
        ...
        try:
            ...
            upload_img(cand, "05_hires_raw", raw)
            # ── Sprint 23C: tattoo-safe restore ───────────────────────────
            # The unmasked re-render above destroys fine tattoo linework (23B
            # finding). Restore the canonical post-enforcement pixels inside the
            # feathered arm masks; everything else keeps the hires re-detail.
            tm = tattoo_masks.get(cand, {})
            if tm:
                from PIL import ImageChops
                union = Image.new("L", (W, H), 0)
                for mk in tm.values():
                    union = ImageChops.lighter(union, mk)  # per-pixel max = mask union
                union_h = union.resize((HW, HH), Image.LANCZOS)
                raw = Image.composite(up, raw, union_h)
                log(f"tattoo-safe restore: {list(tm.keys())}")
            current = raw
            upload_img(cand, "05b_hires_tattoo_safe", current)
        except Exception as e:  # noqa: BLE001
