#!/usr/bin/env python3
"""Sprint 23A — CANON GROUNDING STRESS TEST driver (throwaway, founder-only).

Ships s23a_canon_stress_pod.py: ONE pod, RealVisXL+LoRA+IP-Adapter loaded once, then
FOUR variants of the identical prompt/seed differing ONLY in grounding config
(A face+body@0.5, B face@0.5, C face@0.8, D face+body@0.8), each with the unchanged
Sprint 15A tattoo enforcement. Manual founder comparison; no production path touched.

Safety: SPEND_CAP=0.10, 1200s in-pod watchdog, 1200s driver wallclock, launch-time
...
SPEND_CAP = 0.10            # S23A: 4 variants in one pod
TERMINATE_AT = 0.09        # external kill threshold (margin below the cap)
WATCHDOG_S = 1200          # in-pod self-kill backstop; 1200s @ $0.22/hr = $0.073 < cap
DRIVER_WALLCLOCK_S = 1200  # driver-side max observed uptime before force-kill
POLL_S = 20
...
def build_report(plan, by_route, pod_id, gpu, st, spend, runtime_s, no_orphans,
    ...
    report = {
        "executor": "s23a_canon_stress_driver",
        "version": "v1",
        "experiment": "s23a_canon_grounding_stress_test",
...
def main():
    ...
    run_id = os.environ.get("FOUNDER_RUN_ID") or ("s23a_stress_" + time.strftime(
    ...
    CANON_GROUNDING.update({"base_model_id": base_model_id,
                            "matrix": {"A": "face+body@0.5", "B": "face@0.5",
                                       "C": "face@0.8", "D": "face+body@0.8"},
    ...
    with open(os.path.join(HERE, "s23a_canon_stress_pod.py"), "rb") as f:
