#!/usr/bin/env python3
"""Founder Async Lite — detached RunPod masked-diffusion driver (Phase 3, Sprint 13).

Spawned by the backend (adult_founder_job_service) as a detached process. Reuses the
PROVEN pod pipeline (comfyui_proof_pod_inpaint.py — UNCHANGED) to produce ONE real
tattoo-enforced final image (99_final), NOT the diagnostic montage.

Differences vs the Sprint 8 enforce driver (founder-only async wiring):
  - $0.05 proof_lib safety profile: SPEND_CAP=0.05, 700s in-pod watchdog, 700s driver
    wallclock, launch-time RATE GUARD (max safe ~$0.244/hr), finally-terminate, orphan
    verify. Triple kill-switch keeps worst-case spend < $0.05.
  - Founder prompt forwarded to the pod via BASE_PROMPT env (the pod already supports it).
  - run_id comes from FOUNDER_RUN_ID; the report is written to
    scripts/founder_reports/{run_id}.json so the backend can reconcile by run_id.

Inputs are CONSUMED FROM THE DB enforcement plan (active LoRA + the two mark crops keyed
by route). Summer only. No UI, no public path, no Canon Studio writes, no training, no
normal generator.
"""
import base64
import json
...
SPEND_CAP = 0.05            # founder one-shot hard ceiling
...
DRIVER_WALLCLOCK_S = 700   # driver-side max observed uptime before force-kill
POLL_S = 20
...
def build_report(plan, by_route, pod_id, gpu, st, spend, runtime_s, no_orphans,
    ...
    report = {
        "executor": "founder_async_runpod_driver",
        "version": "v1",
        "run_id": run_id,
        "character_id": SUMMER,
...
def main():
    ...
    run_id = os.environ.get("FOUNDER_RUN_ID") or ("founder_" + time.strftime(
