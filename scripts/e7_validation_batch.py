"""Sprint E7 — neckline boundary fix validation batch (E6 rerun, same conditions).

Runs N prompts sequentially through the PRODUCTION async job path
(start_editor_job → detached driver → RunPod E7 pod → quality gate),
same Summer source image each time, and collects per-run metrics into
scripts/e7_reports/e7_batch_results.json. Downloads each final image to
scripts/e7_reports/<n>_final.png for visual inspection.
...
CHARACTER_ID = 60
REPORT_DIR = Path(__file__).parent / "e7_reports"
...
def main() -> int:
    ...
    try:
        ...
        for idx in indices:
            ...
            rows.append(row)
            out = REPORT_DIR / f"e7_batch_results{'_retry' if tag else ''}.json"
