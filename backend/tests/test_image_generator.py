"""Tests for the B17 simplified image generator endpoint with provider toggle."""
import json
...
def test_include_character_locked_succeeds(client: TestClient):
    """include_character=True works for a locked character."""
...
def test_include_character_injects_identity_into_prompt(client: TestClient):
    """When include_character=True, the identity lock string is appended to the prompt
    ...
    assert resp.status_code == 200, resp.text
    # Identity lock string should have been appended to the original prompt
    assert "At a masquerade ball" in captured.get("prompt", ""), (
        f"Original prompt not found in provider prompt: {captured.get('prompt')!r}"
    )
...
def test_include_character_false_skips_identity_injection(client: TestClient):
    ...
    with no identity lock string appended."""
    ...
    assert resp.status_code == 200, resp.text
    # The prompt sent to the provider should be exactly the user's prompt (no identity appended)
